...
const DetailScreen = ({ route, navigation }: { route: DetailScreenRouteProp; navigation: DetailScreenNavigationProp }) => {
	...
	return (
		<KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
			<View style={styles.header}>
				...
				<Text style={styles.title}>{entry && entry.user ? (entry.user.id === user.data.id ? 'Your Words' : `${entry.user.username}'s Words`) : 'Loading...'}</Text>
			</View>
						...
						{entry.comments.map((item) => (
							<View key={item.id} style={styles.comment}>
								<TouchableOpacity onPress={() => navigateToUserProfile(item.user.id)}>
									<Image source={{ uri: process.env.API_URL + item.user.profileImageUrl }} />
									<ProfilePicture profileImageUrl={item.user?.profileImageUrl} username={item.user?.username} size={50} hasMarginRight />
								</TouchableOpacity>
			...
			<View style={styles.addCommentContainer}>
				<TextInput placeholder='Add a comment...' placeholderTextColor='#aaa' value={commentText} onChangeText={setCommentText} style={styles.commentInput} />
